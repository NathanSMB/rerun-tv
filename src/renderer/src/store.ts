...
export const useStore = create<AppState>((set, get) => ({
    ...
    async refreshChannels() {
        ...
        const editingStillThere = channels.some(
            (c) => c.channel.id === editingChannelId,
        );
        set({
            channels,
            selectedChannelId: stillThere
                ? selectedChannelId
                : (channels[0]?.channel.id ?? null),
    ...
    prewarm() {
        return serialize(async () => {
            ...
            if (
                !current ||
                get().pendingNext !== null ||
                !get().settings.prewarmNext
            )
                return;
    ...
    leavePlayer() {
        return serialize(async () => {
            ...
            if (
                pending &&
                (!current || pending.channelId !== current.channelId)
            ) {
                await api.player.release(pending.channelId, pending.episode.id);
            }
    ...
    armSleep(minutes) {
        if (minutes === null)
            return set({ sleepUntil: null, sleepMinutes: null });
        ...
        set({
            sleepUntil: Date.now() + clamped * 60_000,
            sleepMinutes: clamped,
        });
    },
    ...
    adjustSleep(deltaMin) {
        ...
        const next = Math.min(
            SLEEP_MAX_MIN,
            Math.round(remainingMin) + deltaMin,
        );
    ...
    sleepNow() {
        return serialize(async () => {
            ...
            await bridge().player.reportEnded(
                current.channelId,
                current.episode.id,
                false,
            );
    ...
    toggleMute() {
        ...
        set({ muted });
        if (get().settings.rememberVolume)
            void bridge().settings.set("muted", muted);
    },
    ...
    async setSetting(key, value) {
        ...
        const dropStandby =
            (key === "prewarmNext" && value === false) || key === "loudnessEq";
        if (dropStandby) {
            const pending = get().pendingNext;
            if (pending) {
                set({ pendingNext: null });
                // That episode's job only — the one on air is on the same channel.
                await bridge().player.release(
                    pending.channelId,
                    pending.episode.id,
                );
            }
