...
export default function SleepPanel({
  ...
  const remaining =
    sleepUntil === null
      ...
      : Math.min(
          SLEEP_MAX_MIN,
          Math.max(0, Math.round((sleepUntil - nowMs) / 60_000 / SLEEP_STEP_MIN) * SLEEP_STEP_MIN)
        )
  ...
  const stopsAt = ((): string => {
    ...
    const suffix = endsUnit ? 'lets the episode finish' : 'lets the arc finish'
  ...
  const pushDigit = useCallback(
    (digit: string) => {
      ...
      if (next === '0' || next.length >= 3 || Number(next) * 10 > SLEEP_MAX_MIN) {
        return commitDigits()
      }
      digitTimerRef.current = window.setTimeout(commitDigits, DIGIT_COMMIT_MS)
    },
    [commitDigits]
  )
  ...
  return (
    <div
      ...
      onWheel={(e) => onAdjust(e.deltaY < 0 ? SLEEP_STEP_MIN : -SLEEP_STEP_MIN)}
    >
      <div className="sp-read">
        <span className={`sp-dur${sleepUntil === null ? ' is-off' : ''}`}>{readout}</span>
      ...
      <Slider
        ...
        verticalStep={30}
        pageStep={30}
        ariaValueText={sleepUntil === null ? 'Off' : formatMinutes(remaining)}
        ...
        {PRESETS.map((preset) => (
          <button
            ...
            className={`sp-chip${
              ...
              (preset.minutes != null && preset.minutes > 0 && !expired && remaining === preset.minutes)
                ? ' is-active'
                : ''
            }`}
