...
export default function ChannelFold({
    channelId,
    onClose,
}: Props): ReactElement {
     ...
     * so focus has to follow. A ref callback rather than `autoFocus`, and a *stable*
     * one: an inline arrow is a new function every render, which would make React
     * re-run it on each keystroke and drag focus back from wherever it had moved.
     */
    ...
    const episodeCounts = useMemo(() => {
        ...
        for (const show of library?.shows ?? [])
            map.set(show.id, show.episodeCount);
        return map;
    }, [library]);
    ...
    function commitName(): void {
        ...
        void run("name", () =>
            window.rerun.channels.update(detail.channel.id, { name }),
        );
    }
    ...
    function commitNumber(): void {
        ...
        void run("number", () =>
            window.rerun.channels.update(detail.channel.id, { number: value }),
        );
    }
    ...
    const setSeasonMode = (
        ...
        if (detail == null) return;
        const current =
            entry.seasons.find((item) => item.season === season)
                ?.modeOverride ?? null;
        if (current === mode) return;
        void run(`season-mode:${entry.showId}:${season}`, () =>
            window.rerun.channels.setSeasonMode(
                detail.channel.id,
                entry.showId,
                season,
                mode,
    ...
    const bumpWeight = (entry: LineupEntry, delta: number): void => {
        ...
        const weight = Math.min(
            MAX_WEIGHT,
            Math.max(MIN_WEIGHT, entry.weight + delta),
        );
        if (weight === entry.weight) return;
        void run(`weight:${entry.showId}`, () =>
            window.rerun.channels.setWeight(
                detail.channel.id,
                entry.showId,
                weight,
            ),
    ...
    const resetProgress = (entry: LineupEntry): void => {
        ...
        void run(`reset:${entry.showId}`, () =>
            window.rerun.channels.resetProgress(
                detail.channel.id,
                entry.showId,
            ),
        );
    ...
    const addShow = (showId: number): void => {
        if (detail == null) return;
        void run(`add:${showId}`, () =>
            window.rerun.channels.addShow(detail.channel.id, showId),
        );
    };
    ...
    function renderProgress(entry: LineupEntry): ReactElement {
            ...
            if (progress.remaining >= progress.total) {
                return (
                    <span className="strip-sub">
                        Shuffle bag: fresh cycle — nothing repeats until all{" "}
                        {progress.total} units air
                    </span>
                );
            }
            return (
                <span className="strip-sub">
                    Shuffle bag: {progress.remaining} of {progress.total} units
                    left this cycle ·{" "}
                    <button
                        type="button"
                        className="linkbtn"
        ...
        if (progress.code == null) {
            return (
                <span className="strip-sub">
                    Cursor at the top of the run — the next airing starts with
                    the pilot
                </span>
            );
        }
        return (
            <span className="strip-sub">
                Cursor at <code>{progress.code}</code> — next airing continues
                from here ·{" "}
                <button
                    type="button"
                    className="linkbtn"
        ...
        <div className="fold-panel">
            <div className="fold-bar">
                <span className="caption">
                    Editing CH {String(channel.number).padStart(2, "0")} ·{" "}
                    {channel.name} — changes air immediately
                </span>
                <button
                    type="button"
            ...
            <div className="fold-cols">
              ...
              the channel itself rather than on its lineup — kept on one line. */}
                        ...
                        {confirmingDelete ? (
                            <span className="del-confirm">
                                <span className="del-warn">
                                    Deletes its lineup and progress —
                                </span>
                                <button
                                    ref={confirmRef}
                                    type="button"
                    ...
                    <div className="caption fold-caption">
                        Lineup — each pick draws a show by weight, then an
                        episode by mode
                    </div>

                    {lineup.length === 0 ? (
                        <p className="fold-empty">
                            <b>This channel has nothing to air.</b> Add a show
                            from the library on the right and the scheduler
                            starts drawing from it immediately.
                        </p>
                    ) : (
                        lineup.map((entry) => (
                            <div className="strip" key={entry.showId}>
                                <span className="strip-name">
                                    ...
                                    <span className="strip-facts">
                                        {entry.episodeCount} EP ·{" "}
                                        {plural(entry.unitCount, "unit")}
                                        {entry.arcSummary != null && (
                                            <span className="arcbadge">
                                                {entry.arcSummary}
                                            </span>
                                        )}
                                    </span>
                                ...
                                <fieldset
                                    className="seg"
                                    aria-label={`Play mode for ${entry.title}`}
                                >
                                    <button
                                        type="button"
                                        className={
                                ...
                                <span className="weight">
                                    ...
                                    <button
                                        ...
                                        disabled={
                                            locked || entry.weight <= MIN_WEIGHT
                                        }
                                    ...
                                    <button
                                        ...
                                        disabled={
                                            locked || entry.weight >= MAX_WEIGHT
                                        }
                                        onClick={() => bumpWeight(entry, 1)}
                                    >
                                        +
                                ...
                                {entry.seasons.length > 0 && (
                                    <details className="season-overrides">
                                        <summary>
                                            ...
                                            {entry.seasons.some(
                                                (season) =>
                                                    season.modeOverride != null,
                                            ) && (
                                                <span className="override-count">
                                                    {
                                                        entry.seasons.filter(
                                                            (season) =>
                                                                season.modeOverride !=
                                                                null,
                                                        ).length
                    ...
                    <p className="fold-facts">
                        {plural(lineup.length, "show")} ·{" "}
                        {plural(totalUnits, "playable unit")} ·{" "}
