...
export default function VideoSurface({
    ...
    /**
     * The narrowed dependency list below is the feature, not an oversight. This
     * effect owns an ffmpeg process, so it may only re-run when the *stream
     * identity* changes — `source.key` / `source.url` — never when the `source`
    ...
    useEffect(() => {
        ...
        const pump = startPump({
            ...
            signal: controller.signal,
            policy: standbyRef.current
                ? STANDBY_BUFFER_POLICY
                : DEFAULT_BUFFER_POLICY,
            promotedPolicy: DEFAULT_BUFFER_POLICY,
            onFallback: (reason) => {
                console.warn(
                    `[player] ${source.url}: ${reason} — falling back to plain playback`,
                );
        ...
        // `standby` is deliberately absent — see `standbyRef`. `generation` is the
        // explicit "rebuild the pipe" signal, and is the only way to force a restart.
    ...
    return (
        // biome-ignore lint/a11y/useMediaCaption: there is no caption track to point at — the source is a local file re-muxed or transcoded by ffmpeg, and the stream server publishes one video and one audio track only
        <video
            ref={setElement}
            className={className}
            playsInline
