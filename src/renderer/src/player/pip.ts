...
const step = (state: PipState, command: PipCommand | null = null): PipStep => ({
    state,
    command,
});
...
export function reducePip(state: PipState, event: PipEvent): PipStep {
    if (event.type === "teardown") {
        ...
        if (state.phase === "idle" || state.phase === "closing")
            return step(PIP_IDLE);
        ...
        case "requesting":
            switch (event.type) {
                ...
                case "failed": {
                    ...
                    return step(
                        PIP_IDLE,
                        state.orphaned ? { type: "returnInline" } : null,
                    );
                }
                case "left":
                ...
                case "swapped":
                    ...
                    if (event.slot === state.slot)
                        return step({ ...state, queued: null });
