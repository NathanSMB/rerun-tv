...
import { detectArcs, type ArcCandidate } from './arcs.js'
...
export class Scanner {
  ...
  async scan(full = false): Promise<void> {
    ...
    try {
      ...
      for (const candidate of candidates) {
        ...
        const showId = await this.#ingest(candidate.path, candidate.root, full, seen, dirty)
  ...
  startWatching(): void {
    ...
    const watcher = watch(roots, {
      ...
      awaitWriteFinish: { stabilityThreshold: WRITE_SETTLE_MS, pollInterval: 200 }
    })
    watcher.on('add', (path) => this.#enqueue(() => this.#ingestWatched(path, roots)))
    watcher.on('change', (path) => this.#enqueue(() => this.#ingestWatched(path, roots)))
  ...
  async #walk(dir: string, root: string, out: Candidate[], visited: Set<string>): Promise<void> {
    if (this.#disposed) return
    let real: string
    try {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        ...
        try {
          ...
          if (info.isDirectory()) await this.#walk(full, root, out, visited)
          else if (info.isFile() && isVideoFile(full)) out.push({ path: full, root })
        } catch {
          continue
        }
  ...
  async #ingest(
    ...
    if (existing && !full && existing.mtimeMs === mtimeMs && existing.sizeBytes === sizeBytes) {
    ...
    const show = upsertShow(this.#db, showTitle, showFolderFor(root, filePath, showTitle))
  ...
  #refreshAutoArcs(showId: number): void {
    ...
    for (const arc of listArcs(this.#db, showId)) {
      if (arc.source === 'manual') for (const id of arc.episodeIds) manualMembers.add(id)
    }
    ...
    const candidates: ArcCandidate[] = listEpisodes(this.#db, showId)
      ...
      .map((e) => ({ id: e.id, season: e.season, episode: e.episode, title: e.title }))
  ...
  async #ingestWatched(filePath: string, roots: string[]): Promise<void> {
    ...
    const showId = await this.#ingest(filePath, root, false, new Set(), new Set())
    if (showId === null) {
  ...
  #refreshAutoArcsIfShowRemains(showId: number): void {
    const show = this.#db.prepare('SELECT 1 FROM shows WHERE id = ?').get(showId)
...
function owningRoot(filePath: string, roots: string[]): string | null {
  ...
  for (const root of roots) {
    ...
    if (!filePath.startsWith(root.endsWith(sep) ? root : root + sep)) continue
