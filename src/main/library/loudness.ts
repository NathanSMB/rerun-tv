...
import { listEpisodesNeedingLoudness, loudnessCoverage, saveLoudness } from '../db/repositories/library.js'
...
export class LoudnessScanner {
  ...
  async #pass(signal: AbortSignal): Promise<void> {
    ...
    const pending = listEpisodesNeedingLoudness(this.#db).filter((row) => !this.#failed.has(row.id))
    ...
    for (const episode of pending) {
      ...
      const result = await measureLoudness(ffmpegPath, episode.path, signal)
      if (signal.aborted) return

      if (result.error !== null) {
        this.#failed.add(episode.id)
        console.warn(`[loudness] skipped ${episode.path}: ${result.error}`)
        continue
      }
