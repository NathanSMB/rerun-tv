 ...
 */
const SXXEXX = /s(\d{1,3})[\s._-]*e(\d{1,3})(?:[\s._-]*(?:s\d{1,3})?e(\d{1,3}))?/i
...
const JUNK_PHRASE = /\b(?:web[\s-]?dl|blu[\s-]?ray|dts[\s-]?hd|dd[p]?[\s-]?\d\.\d)\b.*$/i
...
function tidyShowName(raw: string): string {
  const withoutBrackets = raw.replace(BRACKETED_YEAR, ' ').replace(BRACKETED_TAG, ' ')
  return collapse(dotsToSpaces(withoutBrackets)).replace(/^[-–—\s.]+|[-–—\s.,]+$/g, '')
}
...
function cleanTitle(tail: string): string | null {
  ...
  const title = collapse(kept.join(' ')).replace(/^[-–—:_\s]+|[-–—:_\s]+$/g, '')
