...
function measurementOf(row: EpisodeRow): LoudnessMeasurement | null {
    const {
        ...
        loudness_tp: tp,
        loudness_lra: lra,
        loudness_thresh: thresh,
    } = row;
    if (i === null || tp === null || lra === null || thresh === null)
        return null;
...
export async function startStreamServer(
    ...
    const server = createServer((req, res) => {
        handle(req, res).catch((err: unknown) => {
            ...
            if (!res.headersSent)
                sendText(res, 500, `stream error: ${message}`);
            else res.destroy();
        });
    });
    ...
    async function handle(
        ...
        const channelId =
            rawChannel !== null && /^\d+$/.test(rawChannel)
                ? Number(rawChannel)
                : null;
        const key =
            channelId !== null
                ? channelKey(channelId, row.id)
                : `episode:${row.id}`;
        ...
        const path = effectivePlaybackPath(
            row.playback_path,
            row.acodec,
            settings.loudnessEq,
        );
            ...
            const args =
                path === "transcode"
                    ? transcodeArgs(
                          ...
                          loudness,
                          usingAccel,
                          getHwAccel().vaapiDevice,
                      )
                    : remuxArgs(
                          row.path,
                          seekS,
                          !needsAudioTranscode(row.acodec),
                          settings,
                          loudness,
                      );

            const child: ChildProcess = supervisor.spawn(
                key,
                args,
                ffmpegPath as string,
                {
                    onExit: (code, signal, stderrTail) => {
                        if (signal !== null) {
                            ...
                            if (!headersSent && !res.writableEnded && !fellBack)
                                res.destroy();
                            return;
                        }
                        if (code === 0) {
    ...
    return {
        ...
        urlFor(episodeId: number, seekS?: number, channelId?: number): string {
            ...
            if (channelId !== undefined)
                url.searchParams.set("ch", String(channelId));
            return url.toString();
        },
        releaseChannel(channelId: number): void {
