...
export function inspectAndStage(
    ...
    if (!stat.isFile())
        throw new Error(`${basename(sourcePath)} is not a file.`);
    ...
    try {
        ...
        if (integrity !== "ok") {
            throw new Error(
                `${basename(sourcePath)} is corrupt and cannot be imported.`,
            );
        }

        sourceSchemaVersion = source.pragma("user_version", {
            simple: true,
        }) as number;
        if (sourceSchemaVersion === 0) {
            throw new Error(
                `${basename(sourcePath)} is not a Rerun TV database.`,
            );
        }
    ...
    try {
        ...
        try {
            ...
            const paths = (
                staged
                    .prepare("SELECT path FROM episodes LIMIT ?")
                    .all(PATH_SAMPLE_SIZE) as {
                    path: string;
                }[]
            ).map((row) => row.path);
            ...
            return {
                sourcePath,
                sourceSchemaVersion,
                schemaVersion: staged.pragma("user_version", {
                    simple: true,
                }) as number,
                shows,
                episodes,
...
function backupCurrent(dbPath: string, backupsDir: string): string | null {
    ...
    try {
        ...
        console.error(
            "[restore] clean backup failed, copying raw files instead:",
            err,
        );
...
export function getRestoreReceipt(db: Db): RestoreReceipt | null {
    const row = db
        ...
        .get("lastRestore") as { value: string } | undefined;
    if (!row) return null;
    try {
