...
class Cdp {
    ...
    while (true) {
      ...
      if (end !== -1) {
        if (!head.subarray(0, end).toString('latin1').includes('101')) {
          throw new Error(`the debugger refused the upgrade: ${head.subarray(0, end)}`)
        }
  ...
  #writeFrame(text) {
    ...
    if (body.length < 126) {
      ...
      header = Buffer.from([0x81, 0xfe, body.length >> 8, body.length & 0xff])
    } else {
...
function launch(options) {
  ...
  const leading = process.env.RERUN_SOAK_BIN
    ...
    : ['electron', '.']
  const args = [...leading, `--remote-debugging-port=${options.port}`, '--mute-audio']
  ...
  child.on('error', (err) => fail(2, `could not launch the app: ${err.message}`))
  return child
}
    ...
    try {
      ...
      const page = targets.find(
        (target) =>
          ...
          /^app:\/\/|\/renderer\/index\.html|localhost:\d+/.test(target.url ?? '')
      )
...
function countEncoders() {
  ...
  return out.stdout
    .split('\n')
    .filter(
      (line) => /^\S*\bffmpeg\S*\s/.test(line) && line.includes('frag_keyframe+empty_moov')
    ).length
}
...
async function soak(cdp, options) {
  ...
  for (let index = 0; index < options.episodes; index++) {
    ...
    for (;;) {
      ...
      if (episodeId === null) {
        ...
        if (probe.episodeId === null || probe.episodeId === lastEpisodeId) {
          await sleep(200)
          continue
        }
        ...
        log(`episode ${index + 1}/${options.episodes} — id ${episodeId}`)
      }
      ...
      overBudgetPolls = encoders > options.maxEncoders ? overBudgetPolls + 1 : 0
      ...
      if ((Date.now() - started) / 1000 > options.episodeTimeoutS) {
        violations.push(`episode ${episodeId}: did not finish within ${options.episodeTimeoutS}s`)
        break
      }
      await sleep(500)
    }
  ...
  log(`peak encoders seen: ${peakEncoders} (budget ${options.maxEncoders} + dying jobs)`)

  const repeats = seenEpisodes.length - new Set(seenEpisodes).size
  if (repeats > 0) {
    violations.push(`${repeats} episode(s) aired twice — the schedule is double-advancing`)
  }
...
async function main() {
  ...
  try {
    ...
    if (options.evaluate !== null) {
      await cdp.send('Runtime.enable')
      console.log(JSON.stringify(await cdp.evaluate(options.evaluate), null, 2))
      exitCode = 0
    } else {
