...
function scanAll(stream: Uint8Array, chunkSize: number): ReturnType<Mp4SegmentScanner['drain']> {
  ...
  for (let at = 0; at < stream.length; at += chunkSize) {
    scanner.push(stream.subarray(at, Math.min(stream.length, at + chunkSize)))
...
function boxTypeOf(segment: Uint8Array, at = 0): string {
  return String.fromCharCode(segment[at + 4], segment[at + 5], segment[at + 6], segment[at + 7])
}
...
function trak(kind: string, description: Uint8Array): Uint8Array {
  return box('trak', box('mdia', hdlr(kind), box('minf', box('stbl', description))))
}
...
function avcEntry(profile: number, compatibility: number, level: number): Uint8Array {
  ...
  fixed[26] = 0
  fixed[27] = 120
  return box('avc1', fixed, box('avcC', bytes(1, profile, compatibility, level)))
}
...
function esds(objectType: number, audioSpecificConfig?: Uint8Array): Uint8Array {
  ...
  const config = new Uint8Array([0x04, configPayload.length, ...configPayload])
...
describe.skipIf(noFfmpeg)('Mp4SegmentScanner against real ffmpeg output', () => {
  it('splits the stream into one init segment and a run of moof+mdat pairs', () => {
    ...
    for (const segment of media) expect(boxTypeOf(segment.data)).toBe('moof')
  ...
  it('is indifferent to how the bytes are chopped up', () => {
    ...
    for (const chunkSize of [1, 3, 7, 511, 4096]) {
      ...
      expect(segments.map((s) => `${s.kind}:${s.data.length}`)).toEqual(
        reference.map((s) => `${s.kind}:${s.data.length}`)
      )
    }
  })
...
describe('Mp4SegmentScanner error handling', () => {
  it('rejects a stream that does not open with ftyp/moov', () => {
    ...
    scanner.push(new Uint8Array([0, 0, 0, 16, ...fourCc('mdat'), 0, 0, 0, 0, 0, 0, 0, 0]))
...
describe('parseInitSegment', () => {
  ...
  it('formats the avcC bytes as RFC 6381 wants them', () => {
    ...
    expect(parseInitSegment(moov(trak('vide', stsd(avcEntry(0x42, 0xe0, 0x1e)))))?.video).toBe(
      'avc1.42E01E'
    )
  })
  ...
  it('names AAC-LC, HE-AAC and mp3 from the esds descriptor tree', () => {
    ...
    expect(withAudio(audioEntry('mp4a', esds(0x40, bytes(0x12, 0x10))))).toBe('mp4a.40.2')
    expect(withAudio(audioEntry('mp4a', esds(0x40, bytes(0x28, 0x10))))).toBe('mp4a.40.5')
    expect(withAudio(audioEntry('mp4a', esds(0x40, bytes(0xe8, 0x10))))).toBe('mp4a.40.29')
  ...
  it('falls back to AAC-LC for an mp4a entry with no readable esds', () => {
    const info = parseInitSegment(moov(avc, trak('soun', stsd(audioEntry('mp4a')))))
    expect(info?.audio).toBe('mp4a.40.2')
  })
  ...
  it('names the other three soundtracks the remux path can carry', () => {
    ...
    expect(withAudio(audioEntry('Opus', box('dOps', zeros(11))))).toBe('opus')
    expect(withAudio(audioEntry('fLaC', box('dfLa', zeros(34))))).toBe('flac')
  ...
  it('returns null rather than guessing at a stream it cannot describe', () => {
    ...
    expect(parseInitSegment(moov(trak('vide', stsd(box('hvc1', zeros(78))))))).toBeNull()
    // A video track with an unreadable avcC.
    expect(parseInitSegment(moov(trak('vide', stsd(box('avc1', zeros(78))))))).toBeNull()
    // Audio present but unnameable: refuse rather than drop the track silently.
    expect(parseInitSegment(moov(avc, trak('soun', stsd(audioEntry('ac-3')))))).toBeNull()
    // Audio-only: these are television episodes.
    expect(parseInitSegment(moov(trak('soun', stsd(audioEntry('mp4a')))))).toBeNull()
  })

  it('ignores tracks that are neither video nor audio', () => {
    const withSubtitles = moov(avc, trak('sbtl', stsd(box('tx3g', zeros(8)))))
    expect(parseInitSegment(withSubtitles)?.mime).toBe('video/mp4; codecs="avc1.640028"')
  })
})
...
class FakeMediaSource implements MediaSourceLike {
  ...
  constructor(private readonly configure: (sb: FakeSourceBuffer) => void = () => {}) {}
...
function scriptedReader(chunks: Uint8Array[], onRead?: () => void): ScriptedReader {
  let index = 0
  return {
    reads: 0,
    cancelled: false,
...
describe.skipIf(noFfmpeg)('startPump', () => {
  ...
  it('evicts the back buffer so renderer memory stays flat across an episode', async () => {
    ...
    const pump = startPump({
      openReader: () => Promise.resolve(scriptedReader(slice(aacStream, 8192))),
      mediaSource,
      clock,
      sleep: noSleep,
    ...
    for (const [, end] of sb.removals) expect(end).toBeLessThanOrEqual(clock.currentTime)
  })
  ...
  it('evicts harder and retries when the buffer reports QuotaExceededError', async () => {
    ...
    const pump = startPump({
      ...
      clock,
      sleep: noSleep,
      policy: { highWaterS: 1e9, backBufferS: 1e9, tightBackBufferS: 10, pollMs: 0 }
    })
    ...
    expect(sb.removals.some(([, end]) => Math.abs(end - 290) < 1e-6)).toBe(true)
    // … and the last resort takes the rest of the back buffer.
    expect(sb.removals.some(([, end]) => Math.abs(end - 300) < 1e-6)).toBe(true)
  ...
  it('falls back rather than failing when the stream defeats the parser', async () => {
    ...
    const junk = new Uint8Array([0, 0, 0, 16, ...fourCc('mdat'), 0, 0, 0, 0, 0, 0, 0, 0])
    const pump = startPump({
      ...
      onError: () => {
        throw new Error('a parse failure must not be reported as an error')
      }
    })
  ...
  it('falls back when the browser refuses the codecs we derived', async () => {
    ...
    const pump = startPump({
      ...
      onError: () => {
        throw new Error('an unsupported codec is a fallback, not an error')
      }
    })
  ...
  it('stays silent when the pump is stopped mid-stream, and frees the reader', async () => {
    ...
    while (reader.reads <= 3) await new Promise((resolve) => setTimeout(resolve, 1))
    pump.stop()
    await pump.done

    expect(reader.cancelled).toBe(true)
    // Only what it had already read — the point is that it stopped early.
    expect(pump.stats.bytes).toBe(chunks.slice(0, 3).reduce((n, c) => n + c.length, 0))
  ...
  it('promote() raises a standby pump to the full read targets', async () => {
    ...
    const reader = scriptedReader(slice(aacStream, 2048), () => {
      const sb = mediaSource.buffers[0]
      if (sb) observations.push({ ahead: sb.bufferedEnd - clock.currentTime, promoted })
    })
    ...
    const whileStandby = observations.filter((o) => !o.promoted).map((o) => o.ahead)
    expect(whileStandby.length).toBeGreaterThan(1)
    expect(Math.max(...whileStandby)).toBeLessThan(STANDBY_BUFFER_POLICY.highWaterS)
    // … and once promoted it read on well past it, which only the full policy allows.
    const afterPromotion = observations.filter((o) => o.promoted).map((o) => o.ahead)
    expect(Math.max(...afterPromotion)).toBeGreaterThan(STANDBY_BUFFER_POLICY.highWaterS)
    expect(mediaSource.ended).toBe(true)
  })
})
