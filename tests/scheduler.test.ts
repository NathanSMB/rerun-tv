...
} from "@main/services/channels.js";
...
// One handle per test, and this file has a lot of tests: closing them keeps the
// count flat instead of letting it grow with the suite.
afterEach(() => {
    db.close();
});
...
describe("arc atomicity", () => {
    ...
    it("locks the channel when the lottery draws an arc, and logs each part", () => {
        ...
        expect(parts.map((p) => p?.episodeId)).toEqual([
            episodeIds[5],
            episodeIds[6],
        ]);
        ...
        expect(logged.map((r) => r.episodeId)).toEqual([
            episodeIds[6],
            episodeIds[5],
            episodeIds[4],
        ]);
...
describe("sequential mode", () => {
    it("advances one unit per pick and wraps to the pilot after the finale", () => {
        ...
        const aired = [0, 1, 2, 3, 4].map(
            () => pickNext(db, channel.id, rng)?.unit.key,
        );
...
describe("shuffle mode", () => {
    it("airs every unit exactly once before any repeats", () => {
        ...
        for (let i = 0; i < 9; i++)
            cycleOne.push(pickNext(db, channel.id, rng)?.unit.key as string);
        const cycleTwo: string[] = [];
        for (let i = 0; i < 9; i++)
            cycleTwo.push(pickNext(db, channel.id, rng)?.unit.key as string);
    ...
    it("never refills a bag that leads with the unit that just aired", () => {
        for (const seed of [1, 2, 3, 4, 5, 6, 7, 8]) {
            ...
            const keys: string[] = [];
            for (let i = 0; i < 10; i++)
                keys.push(pickNext(fresh, channel.id, rng)?.unit.key as string);
    ...
    it("drops bag keys whose units no longer exist", () => {
        ...
        db.prepare(`DELETE FROM episodes WHERE id = ?`).run(
            Number(stale[0].slice("ep:".length)),
        );

        const next = pickNext(db, channel.id, rng);
        expect(next?.unit.key).not.toBe(stale[0]);
        expect(getShowState(db, channel.id, showId).shuffleBag).not.toContain(
            stale[0],
        );
    });
});
...
describe("season mode overrides", () => {
    it("keeps an ordered season in order inside an otherwise shuffled cycle", () => {
        ...
        const cycle = Array.from(
            { length: 10 },
            () => pickNext(db, channel.id, rng)?.unit,
        );
        ...
        expect(
            shuffledEpisodes.slice().sort((a, b) => (a ?? 0) - (b ?? 0)),
        ).toEqual([1, 2, 3, 4, 5]);
    ...
    it("removes an override to inherit again and exposes effective modes to the editor", () => {
        ...
        expect(listChannelShowSeasonModes(db, channel.id, showId)).toHaveLength(
            1,
        );
        ...
        expect(
            getChannelDetail(db, channel.id)?.lineup[0].seasons[1],
        ).toMatchObject({
            modeOverride: null,
            effectiveMode: "shuffle",
        });
...
describe("peekNext", () => {
    it("is side-effect free and agrees with the pick that follows", () => {
        ...
        const peeks = [
            peekNext(db, channel.id, seeded(8)),
            peekNext(db, channel.id, seeded(8)),
        ];
...
describe("channel view models", () => {
    ...
    it("falls back to a plain arc count when no single arc dominates", () => {
        ...
        expect(getChannelDetail(db, channel.id)?.lineup[0].arcSummary).toBe(
            "2 ARCS",
        );
    });
...
describe("prewarm reservations", () => {
    function playLogCount(): number {
        return (
            db.prepare(`SELECT COUNT(*) AS n FROM play_log`).get() as {
                n: number;
            }
        ).n;
    }
    ...
    it("reserving commits nothing and repeats the same pick until resolved", () => {
        ...
        expect(peekNext(db, channel.id, seeded(7))?.episodeId).toBe(
            first?.episodeId,
        );
    });
    ...
    it("promoting spends the reservation exactly as pickNext would have", () => {
        ...
        expect(reserveNext(db, channel.id, seeded(1))?.episodeId).not.toBe(
            reserved.episodeId,
        );
    });
    ...
    it("a discarded reservation leaves cursor, bag, arc lock and log untouched", () => {
        ...
        expect(pickNext(db, channel.id, seeded(1))?.episodeId).toBe(
            reserved.episodeId,
        );
    });
