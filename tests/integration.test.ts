...
import { openDatabase, type Db } from '../src/main/db/index.js'
import { addScanRoot, listArcs, listEpisodes, listShows } from '../src/main/db/repositories/library.js'
import {
  addChannelShow,
  createChannel,
  setChannelShowMode
...
import { pickNext, peekNext } from '../src/main/scheduler/scheduler.js'
import { buildUnits } from '../src/main/scheduler/units.js'
import { startStreamServer, type StreamServer } from '../src/main/stream/server.js'
...
async function makeClip(path: string): Promise<void> {
  await execFileAsync(ff.ffmpegPath as string, [
    '-y',
    '-loglevel', 'error',
    '-f', 'lavfi', '-i', 'testsrc=d=1:s=128x96:r=10',
...
describe.skipIf(!haveFfmpeg)('scan → schedule → stream', () => {
  ...
  beforeAll(async () => {
    ...
    await makeClip(join(gargoyles, 'Gargoyles - S01E01 - Awakening Part 1.mkv'))
    await makeClip(join(gargoyles, 'Gargoyles - S01E02 - Awakening Part 2.mkv'))
    await makeClip(join(gargoyles, 'Gargoyles - S01E03 - Awakening Part 3.mkv'))
    await makeClip(join(gargoyles, 'Gargoyles - S01E04 - Enter Macbeth.mkv'))
  ...
  it('parses shows from folder names and episodes from filenames', () => {
    const shows = listShows(db).map((s) => s.title).sort()
  ...
  it('stores the playback decision at scan time', () => {
    ...
    expect(listEpisodes(db, gargoyles.id).every((e) => e.playbackPath === 'remux')).toBe(true)
  ...
  it('routes an unparseable file to the Unmatched bucket instead of dropping it', () => {
    const rows = db.prepare('SELECT path FROM unmatched_files').all() as { path: string }[]
    expect(rows.some((r) => r.path.endsWith('gargoyles_special (1).mp4'))).toBe(true)
  })
  ...
  it('serves the direct path with range support', async () => {
    ...
    const ranged = await fetch(server.urlFor(episode.id), { headers: { Range: 'bytes=0-9' } })
    expect(ranged.status).toBe(206)
    expect(ranged.headers.get('content-range')).toBe(`bytes 0-9/${body.byteLength}`)
    expect(new Uint8Array(await ranged.arrayBuffer())).toEqual(body.slice(0, 10))
  })
