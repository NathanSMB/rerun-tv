...
import type { AppSettings, Channel, ChannelDetail, ChannelSummary } from '@shared/types.js'
  ...
  return {
    channels: {
      ...
      reorder: async (ids: number[]) => {
        ...
        channels = ids.flatMap((id) => channels.filter((c) => c.channel.id === id))
      },
      update: async (id: number, patch: Record<string, unknown>) => {
        calls.push({ call: 'update', id, patch })
        channels = channels.map((c) =>
          c.channel.id === id ? { ...c, channel: { ...c.channel, ...patch } } : c
        )
        return detail(id, channels[0].channel.number, channels[0].channel.name)
      },
      setMode: async (channelId: number, showId: number, mode: string) => {
        calls.push({ call: 'setMode', channelId, showId, mode })
      },
      setWeight: async (channelId: number, showId: number, weight: number) => {
...
const rows = (): HTMLElement[] => Array.from(document.querySelectorAll<HTMLElement>('.ch-row'))
...
const foldHeading = (): string | null =>
  document.querySelector<HTMLElement>('.fold-bar .caption')?.textContent ?? null
...
const press = async (key: string, altKey = false): Promise<void> => {
  const target = document.querySelector<HTMLElement>('.ch-row.sel') ?? rows()[0]
...
beforeEach(async () => {
  ...
  channels = [summary(1, 3, 'Saturday Morning'), summary(2, 7, 'Late Night Static')]
  removed = []
  created = []
  calls = []
  tuned = []
...
describe('the guide rows', () => {
  ...
  it('will not offer to tune in to a channel with nothing on deck', async () => {
    channels = [{ ...summary(1, 3, 'Saturday Morning'), onDeck: null }, channels[1]]
    await act(async () => {
      useStore.setState({ channels })
    })
    expect(button('Tune in to CH 03 Saturday Morning')).toHaveProperty('disabled', true)
  })
...
describe('opening the fold', () => {
  it('unfolds the editor under the row that was pressed', async () => {
    ...
    expect(rows()[0].parentElement?.querySelector('.fold.open')).toBeTruthy()
  })
...
describe('deleting a channel', () => {
  ...
  it('deletes on the second press and folds shut', async () => {
    ...
    expect(useStore.getState().editingChannelId).toBeNull()
    expect(foldHeading()).toBeNull()
    expect(useStore.getState().channels.map((c) => c.channel.id)).toEqual([2])
  })
...
describe('the fold and the guide’s keyboard map', () => {
  ...
  it('does not read typing in the fold as a guide shortcut', async () => {
    await click(button('Edit CH 03 Saturday Morning'))
    const search = document.querySelector<HTMLInputElement>('.fold-right .search')
  ...
  it('leaves Escape to the field that wants it', async () => {
    ...
    const input = document.querySelector<HTMLInputElement>('.fold-idline .id-input')
  ...
  it('commits a rename on Enter', async () => {
    ...
    expect(calls).toContainEqual({ call: 'update', id: 1, patch: { name: 'Weekend Mornings' } })
  })
...
describe('the lineup controls', () => {
  it('wires the scheduling knobs to the channel', async () => {
    ...
    expect(calls).toContainEqual({ call: 'setMode', channelId: 1, showId: 1, mode: 'sequential' })

    await click(button('Raise the weight of Gargoyles'))
    expect(calls).toContainEqual({ call: 'setWeight', channelId: 1, showId: 1, weight: 3 })

    await click(button('Remove Gargoyles from this channel'))
    expect(calls).toContainEqual({ call: 'removeShow', channelId: 1, showId: 1 })
  })

  it('adds a show from the library column', async () => {
    await click(button('Edit CH 03 Saturday Morning'))
    await click(button('Add Animaniacs to this channel'))
    expect(calls).toContainEqual({ call: 'addShow', channelId: 1, showId: 6 })
  })

  it('resets the scheduler’s progress for one show', async () => {
    await click(button('Edit CH 03 Saturday Morning'))
    await click(byText('reshuffle') as HTMLElement)
    expect(calls).toContainEqual({ call: 'resetProgress', channelId: 1, showId: 1 })
  })
})
...
describe('the retired channels screen', () => {
  it('is gone from the nav', async () => {
    ...
    const labels = Array.from(bar.querySelectorAll('.appnav button')).map((el) =>
      el.textContent?.trim()
    )
...
describe('creating a channel', () => {
  it('opens the new channel’s lineup straight away', async () => {
    ...
    const input = document.querySelector<HTMLInputElement>('.guide-newch .textinput')
