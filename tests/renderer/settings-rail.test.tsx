...
async function scrollTo(scrollTop: number): Promise<void> {
  ...
  for (const [id, top] of Object.entries(TOPS)) {
    ...
    node.getBoundingClientRect = () => ({ top: top - scrollTop }) as DOMRect
  }
  await act(async () => {
...
function tunedStop(): string | null {
  const current = container.querySelector('.set-rail-item[aria-current="true"]')
...
describe('the tuning rail', () => {
  ...
  it('tunes to a section by its number', async () => {
    ...
    for (const id of Object.keys(TOPS)) {
      const node = document.getElementById(id)
      if (node != null) node.scrollIntoView = () => void (jumped = id)
    }
    const stops = [...container.querySelectorAll<HTMLButtonElement>('.set-rail-item')]
    await act(async () => {
      stops[3].click()
    })
