...
export type BridgeCall =
  ...
  | { call: 'reportEnded'; channelId: number; episodeId: number; completed: boolean }
...
export function standaloneDeck(count: number): NowPlaying[] {
  return Array.from({ length: count }, (_, index) => playing(episodeView(index), null))
}
  ...
  const player: RerunApi['player'] = {
    ...
    promoteNext: async (channelId, episodeId) => {
      ...
      if (reserved !== episodeId) {
        throw new Error(`promoteNext(${episodeId}) but the reservation was ${reserved}`)
      }
      aired += 1
    },
    peekNext: async () => deck[aired]?.episode ?? null,
    reportEnded: async (channelId, episodeId, completed) => {
      calls.push({ call: 'reportEnded', channelId, episodeId, completed })
    },
    release: async (channelId, episodeId) => {
      calls.push({ call: 'release', channelId, episodeId: episodeId ?? null })
    }
  }
