...
function episode(showId: number, season: number, ep: number, patch: Partial<EpisodeInput> = {}): EpisodeInput {
  return {
    showId,
    season,
    episode: ep,
...
describe('episodes', () => {
  it('inserts, then updates in place on the second upsert of the same path', () => {
    ...
    const again = upsertEpisode(db, episode(show.id, 1, 1, { durationS: 1400, mtimeMs: 2000 }))
  ...
  it('exposes the rescan key so an unchanged file can be skipped', () => {
    const show = upsertShow(db, 'Show', '/tv/Show')
    upsertEpisode(db, episode(show.id, 1, 1, { mtimeMs: 111, sizeBytes: 222 }))
  ...
  it('preserves arc membership across a rescan upsert', () => {
    const show = upsertShow(db, 'Show', '/tv/Show')
    const ids = [1, 2].map((n) => upsertEpisode(db, episode(show.id, 1, n)))
    createArc(db, show.id, 'Awakening', ids, 'manual')

    upsertEpisode(db, episode(show.id, 1, 1, { mtimeMs: 9999 }))

    expect(getEpisode(db, ids[0])).toMatchObject({ partIndex: 1, mtimeMs: 9999 })
  ...
  it('lists in airing order and batch-fetches in airing order', () => {
    ...
    expect(getEpisodesByIds(db, [c, b, a]).map((e) => e.id)).toEqual([a, b, c])
...
describe('arcs', () => {
  function showWithEpisodes(n: number): { showId: number; ids: number[] } {
    const show = upsertShow(db, 'Show', '/tv/Show')
    const ids: number[] = []
    for (let i = 1; i <= n; i++) ids.push(upsertEpisode(db, episode(show.id, 1, i)))
  ...
  it('round-trips creation: members get part indexes in airing order', () => {
    ...
    const arc = createArc(db, showId, 'Awakening', [ids[2], ids[0], ids[1]], 'auto')
    ...
    expect(getEpisode(db, ids[2])).toMatchObject({ partGroupId: arc.id, partIndex: 3 })
    expect(getEpisode(db, ids[3])).toMatchObject({ partGroupId: null, partIndex: null })
  ...
  it('deletion releases the members', () => {
    ...
    for (const id of ids) {
      expect(getEpisode(db, id)).toMatchObject({ partGroupId: null, partIndex: null })
    }
  })
  ...
  it('rejects episodes from another show and empty input', () => {
    ...
    const stray = upsertEpisode(db, episode(other.id, 1, 1, { path: '/tv/Other/S1E1.mkv' }))
    expect(() => createArc(db, showId, 'Nope', [...ids, stray], 'manual')).toThrow()
  ...
  it('spans a double episode when computing the range', () => {
    ...
    const b = upsertEpisode(db, episode(show.id, 1, 3, { path: '/tv/Show/S1E3.mkv' }))
  ...
  it('clearAutoArcs removes auto arcs and leaves manual ones alone', () => {
    ...
    const manual = createArc(db, showId, 'Manual', [ids[2], ids[3]], 'manual')

    clearAutoArcs(db, showId)

    expect(listArcs(db, showId).map((a) => a.id)).toEqual([manual.id])
    expect(getEpisode(db, ids[0])).toMatchObject({ partGroupId: null, partIndex: null })
    expect(getEpisode(db, ids[3])).toMatchObject({ partGroupId: manual.id, partIndex: 2 })
  ...
  it('regrouping moves members and drops the group left empty', () => {
    ...
    const second = createArc(db, showId, 'Manual', [ids[0], ids[1], ids[2]], 'manual')
...
describe('scan roots and unmatched files', () => {
  it('adds roots idempotently and removes them', () => {
    ...
    expect(listScanRoots(db).map((r) => r.path)).toEqual(['/tv', '/movies'])
  ...
  it('upserts unmatched files by path and removes them by id', () => {
    ...
    expect(file).toMatchObject({ reason: 'ffprobe failed', mtimeMs: 3, sizeBytes: 4 })
