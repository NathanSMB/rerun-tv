...
import { listEpisodesNeedingLoudness, loudnessCoverage, saveLoudness } from '@main/db/repositories/library.js'
...
import { remuxArgs, startStreamServer, transcodeArgs, type StreamServer } from '@main/stream/server.js'
import { effectivePlaybackPath, loudnessEqApplies } from '@shared/playback.js'
import { DEFAULT_SETTINGS, type AppSettings, type LoudnessMeasurement } from '@shared/types.js'
...
const QUIET: LoudnessMeasurement = { i: -22.8, tp: -3.1, lra: 15.2, thresh: -33.1 }
...
describe('preGainDb', () => {
  ...
  it('pulls a loud one down', () => {
    expect(preGainDb({ i: -11, tp: -0.2, lra: 6, thresh: -21 })).toBeCloseTo(-5, 2)
  })
  ...
  it('lifts a quiet-but-peaky master the full distance anyway', () => {
    const peaky: LoudnessMeasurement = { i: -24, tp: -0.5, lra: 20, thresh: -34 }
  ...
  it('never moves a soundtrack absurdly far on one measurement', () => {
    expect(preGainDb({ i: -70, tp: -60, lra: 1, thresh: -80 })).toBeLessThanOrEqual(24)
    expect(preGainDb({ i: 12, tp: 6, lra: 1, thresh: 2 })).toBeGreaterThanOrEqual(-24)
  })
})
...
`
  ...
  it('reads the input measurements out of ffmpeg’s stderr', () => {
    expect(parseLoudnessJson(REAL_OUTPUT)).toEqual({ i: -22.8, tp: -3.1, lra: 15.2, thresh: -33.1 })
  })
...
describe('planLoudness', () => {
  ...
  it('skips an inaudible pre-gain rather than spending a filter on it', () => {
    const onTarget: LoudnessMeasurement = { i: -16.02, tp: -2, lra: 8, thresh: -26 }
    expect(planLoudness(ON, 'ac3', onTarget)?.some((f) => f.startsWith('volume='))).toBe(false)
  })
})
...
describe('arg builders with loudness on', () => {
  ...
  it('puts every filter in one -af, with the layout guard still last', () => {
    ...
    expect(chain.at(-1)).toBe('aformat=channel_layouts=mono|stereo|3.0|4.0|5.0|5.1|7.1')
  ...
  it('still copies both streams when the setting is off', () => {
    const args = remuxArgs('/tv/ep.mp4', 0, true, OFF, planLoudness(OFF, 'aac', QUIET))
...
describe('effectivePlaybackPath', () => {
  ...
  it('never changes the other two paths, which already run through ffmpeg', () => {
    ...
    expect(effectivePlaybackPath('transcode', 'ac3', true)).toBe('transcode')
  })
})
...
describe.skipIf(noFfmpeg)('measuring a real file', () => {
  ...
  it('answers "nothing to measure" for a silent track without failing', async () => {
    const result = await measureLoudness(ffmpeg.ffmpegPath as string, silent)
    expect(result.error).toBeNull()
  ...
  it('reports a broken file as an error rather than a measurement', async () => {
    ...
    const result = await measureLoudness(ffmpeg.ffmpegPath as string, broken)
    expect(result.measurement).toBeNull()
...
describe.skipIf(noFfmpeg)('serving an equalized stream', () => {
  ...
  function clip(name: string, acodec: string, playbackPath: string, episode: number): number {
    const path = join(dir, name)
    execFileSync(ffmpeg.ffmpegPath as string, [
      '-hide_banner',
      '-loglevel',
  ...
  it('takes effect on the next tune-in, with no restart', async () => {
    ...
    settings = { ...DEFAULT_SETTINGS, loudnessEq: false }
    expect((await fetch(server.urlFor(directId))).headers.get('accept-ranges')).toBe('bytes')
    settings = { ...DEFAULT_SETTINGS, loudnessEq: true }
...
describe.skipIf(noFfmpeg)('LoudnessScanner', () => {
  ...
  function seed(count: number, acodec = 'pcm_s16le'): void {
    ...
    for (let i = 0; i < count; i++) insert.run(i, join(dir, `job-${i}.wav`), acodec)
  }
  ...
  async function until(done: () => boolean, timeoutMs = 30_000): Promise<void> {
    ...
    while (!done()) {
      if (Date.now() > deadline) throw new Error('timed out waiting for the measuring job')
  ...
  it('records a file it cannot measure without stalling on it', async () => {
    ...
    scanner.start()
    await until(() => !scanner.running && loudnessCoverage(db).measured >= 1)
