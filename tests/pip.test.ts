...
function run(events: PipEvent[]): { commands: PipCommand[]; machine: ReturnType<typeof createPipMachine> } {
...
describe('opening a session', () => {
  ...
  it('goes back to idle when the request is refused', () => {
    ...
    const { commands, machine } = run([{ type: 'toggle', slot: 'a' }, { type: 'failed' }])
  ...
  it('does nothing at a handoff when nothing is floating', () => {
    const { commands } = run([{ type: 'swapped', slot: 'b' }, { type: 'left', slot: 'a' }])
...
describe('the handoff', () => {
  ...
  it('does not move a session that is already on the promoted surface', () => {
    const { commands } = run([...opened('b'), { type: 'swapped', slot: 'b' }])
...
describe('closing', () => {
  ...
  it('treats the window’s own ✕ exactly like the OSD button', () => {
    ...
    const { commands, machine } = run([...opened('a'), { type: 'left', slot: 'a' }])
...
describe('teardown', () => {
  it('closes a live session without steering navigation back to the player', () => {
    ...
    const { commands, machine } = run([...opened('a'), { type: 'teardown' }])

    expect(commands).toEqual([{ type: 'enter', slot: 'a' }, { type: 'exit' }])
    expect(isFloating(machine.state)).toBe(false)
  })

  it('closes a session that is still opening', () => {
    const { commands } = run([{ type: 'toggle', slot: 'a' }, { type: 'teardown' }])
  ...
  it('exits at most once however many times it is asked', () => {
    ...
    expect(commands.filter((command) => command.type === 'exit')).toHaveLength(1)
  })
})
...
describe('the ordering invariant', () => {
    ...
    for (const sequence of sequences(4)) {
      ...
      for (const event of sequence) {
        ...
        if (event.type === 'swapped' && next.command?.type === 'exit') {
          throw new Error(`a swap asked to exit: ${JSON.stringify(sequence)}`)
        }
        state = next.state
      }
      checked += 1
  ...
  it('only ever asks for a gesture-requiring entry on a press', () => {
    for (const sequence of sequences(3)) {
      ...
      for (const event of sequence) {
        const next = reducePip(state, event)
        if (next.command?.type === 'enter') expect(event.type).toBe('toggle')
        state = next.state
      }
    }
  })
