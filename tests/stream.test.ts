...
beforeAll(async () => {
  ...
  db.prepare('INSERT INTO shows (id, title, folder_path, added_at) VALUES (1, ?, ?, ?)').run(
    'Test Show',
    dir,
    Date.now()
  )
  ...
  directId = insertEpisode(directPath, 'direct', 'mov,mp4,m4a,3gp,3g2,mj2', 1, 1)
...
describe('urlFor', () => {
  it('is the one URL shape the <video> element sees', () => {
    expect(server.urlFor(directId)).toBe(`http://127.0.0.1:${server.port}/stream/${directId}`)
  })

  it('carries a seek as ?t= and a channel slot as ?ch=', () => {
    expect(server.urlFor(7, 90)).toBe(`http://127.0.0.1:${server.port}/stream/7?t=90`)
    expect(server.urlFor(7, 0)).toBe(`http://127.0.0.1:${server.port}/stream/7`)
    expect(server.urlFor(7, 12.5, 3)).toBe(`http://127.0.0.1:${server.port}/stream/7?t=12.5&ch=3`)
  })
})
...
describe('direct path', () => {
  ...
  it('answers a bounded range with 206 and the exact bytes', async () => {
    const res = await fetch(server.urlFor(directId), { headers: { Range: 'bytes=0-9' } })
  ...
  it('answers an open-ended range', async () => {
    const res = await fetch(server.urlFor(directId), { headers: { Range: 'bytes=990-' } })
    expect(res.status).toBe(206)
  ...
  it('answers a suffix range', async () => {
    const res = await fetch(server.urlFor(directId), { headers: { Range: 'bytes=-5' } })
    expect(res.status).toBe(206)
  ...
  it('clamps a range that runs past the end', async () => {
    const res = await fetch(server.urlFor(directId), { headers: { Range: 'bytes=995-99999' } })
    expect(res.status).toBe(206)
    expect(res.headers.get('content-range')).toBe('bytes 995-999/1000')
  })

  it('rejects an unsatisfiable range with 416', async () => {
    const res = await fetch(server.urlFor(directId), { headers: { Range: 'bytes=5000-6000' } })
...
describe('routing and access control', () => {
  ...
  it('404s a path that is not an episode id', async () => {
    const res = await fetch(`http://127.0.0.1:${server.port}/stream/../../etc/passwd`)
    expect(res.status).toBe(404)
  })
...
describe('FfmpegSupervisor', () => {
  ...
  function spawnSleeper(sup: FfmpegSupervisor, key: string): ReturnType<FfmpegSupervisor['spawn']> {
    return sup.spawn(key, ['-e', 'setTimeout(() => {}, 60000)'], process.execPath)
  }
  ...
  it('runs one job per key — a second spawn kills the first', async () => {
    ...
    const died = new Promise<void>((resolve) => first.once('exit', () => resolve()))
  ...
  it('drops the slot when a job exits on its own', async () => {
    const sup = new FfmpegSupervisor()
    const child = sup.spawn('one-shot', ['-e', 'process.exit(0)'], process.execPath)
