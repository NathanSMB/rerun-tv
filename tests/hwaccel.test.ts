...
import { DEFAULT_SETTINGS, type HardwareAccel, type HwAccelReport } from '@shared/types.js'
...
const PENDING: HwAccelReport = { vaapi: 'pending', nvenc: 'pending', vaapiDevice: null }
...
describe('hwDecodeArgs', () => {
  ...
  it('asks CUDA for a decode without pinning frames to the GPU', () => {
    ...
    expect(hwDecodeArgs('nvenc', null)).not.toContain('-hwaccel_output_format')
  })
})
...
describe('hwVideoArgs', () => {
  ...
  it('builds a VAAPI chain that survives a software decode and 10-bit input', () => {
    ...
    expect(args).toContain('-vf format=nv12|vaapi,hwupload,scale_vaapi=format=nv12')
  ...
  it('never puts -pix_fmt on the VAAPI path, where the format is the surface’s', () => {
    expect(hwVideoArgs('vaapi', DEFAULT_SETTINGS)).not.toContain('-pix_fmt')
  })
...
describe('qualityTier', () => {
  ...
  it('keeps VAAPI near libx264’s bitrate rather than a few points looser', () => {
    ...
    for (const { tier } of [23, 21, 20, 19].map((crf) => ({ tier: qualityTier(crf) }))) {
...
describe('effectiveAccel', () => {
  ...
  it('degrades a backend this version has never heard of', () => {
    expect(effectiveAccel('quicksync' as HardwareAccel, ALL_OK)).toBe('software')
  })
...
describe('probeHardwareAccel', () => {
  ...
  it('reports failure when no node can encode', async () => {
    ...
    expect(report).toEqual({ vaapi: 'failed', nvenc: 'failed', vaapiDevice: null })
  })
  ...
  it.skipIf(noFfmpeg)('answers without throwing against real hardware', async () => {
    ...
    if (report.vaapi === 'ok') expect(report.vaapiDevice).toMatch(/^\/dev\/dri\/renderD\d+$/)
...
describe('transcodeArgs across backends', () => {
  ...
  it('keeps the mux, the maps and the chapter drop on every backend', () => {
    for (const accel of ['software', 'vaapi', 'nvenc'] as HardwareAccel[]) {
      const args = transcodeArgs('/tv/ep.mkv', 0, DEFAULT_SETTINGS, null, accel, '/dev/dri/renderD129')
      const joined = args.join(' ')
      expect(joined).toContain('-map 0:v:0')
      expect(joined).toContain('-map 0:a:0?')
      expect(joined).toContain('-map_chapters -1')
      expect(joined).toContain('-movflags frag_keyframe+empty_moov+default_base_moof')
  ...
  it('encodes audio identically on every backend', () => {
    for (const accel of ['software', 'vaapi', 'nvenc'] as HardwareAccel[]) {
      const joined = transcodeArgs('/tv/ep.mkv', 0, DEFAULT_SETTINGS, null, accel).join(' ')
      expect(joined).toContain(`-c:a aac -b:a ${DEFAULT_SETTINGS.transcodeAudioBitrate}`)
    }
  })
  ...
  it('puts the decode prefix and the seek ahead of the input', () => {
    const args = transcodeArgs('/tv/ep.mkv', 90, DEFAULT_SETTINGS, null, 'vaapi', '/dev/dri/renderD129')
  ...
  it('composes the loudness chain the same way on every backend', () => {
    for (const accel of ['software', 'vaapi', 'nvenc'] as HardwareAccel[]) {
      ...
      expect(joined).toContain('-af volume=3.00dB,loudnorm=I=-16,aformat=channel_layouts=')
