...
function apply(keep?: number): ReturnType<typeof applyStagedImport> {
  return applyStagedImport({ dbPath, stagedPath, metaPath, backupsDir, keep })
}

function backupFiles(): string[] {
  return existsSync(backupsDir)
    ? readdirSync(backupsDir).filter((n) => n.startsWith('library-pre-restore-') && n.endsWith('.db'))
    : []
}
...
describe('inspectAndStage — rejection', () => {
  it('rejects a file that is not a database', () => {
    ...
    expect(() => inspectAndStage(source, stagedPath, metaPath)).toThrow(/not a SQLite database/)
  ...
  it('rejects an empty file', () => {
    ...
    expect(() => inspectAndStage(source, stagedPath, metaPath)).toThrow(/empty/)
  })

  it('rejects a file that does not exist', () => {
    expect(() => inspectAndStage(join(dir, 'nope.db'), stagedPath, metaPath)).toThrow(
      /could not be read/
    )
  })
  ...
  it('rejects a SQLite database that was never a Rerun TV one', () => {
    ...
    expect(() => inspectAndStage(source, stagedPath, metaPath)).toThrow(/not a Rerun TV database/)
    expect(existsSync(stagedPath)).toBe(false)
  })
  ...
  it('rejects a Rerun-shaped database made by a newer version', () => {
    ...
    expect(() => inspectAndStage(source, stagedPath, metaPath)).toThrow(/newer version of Rerun TV/)
    expect(existsSync(stagedPath)).toBe(false)
  })
  ...
  it('rejects a versioned database missing our tables', () => {
    ...
    expect(() => inspectAndStage(source, stagedPath, metaPath)).toThrow(/not a Rerun TV one/)
    expect(existsSync(stagedPath)).toBe(false)
...
describe('inspectAndStage — acceptance', () => {
  it('stages a current database and reports its contents', () => {
    ...
    seedDatabase(source, (db) => {
      db.prepare('INSERT INTO shows (title, folder_path, added_at) VALUES (?, ?, 0)').run(
        'Frasier',
        '/media/frasier'
      )
      db.prepare('INSERT INTO channels (name, number) VALUES (?, ?)').run('Comfort', 1)
    })
    ...
    expect(JSON.parse(readFileSync(metaPath, 'utf8')).sourcePath).toBe(source)
  })
  ...
  it('migrates an older backup forward while staging it', () => {
    ...
    const tables = (
      staged.prepare("SELECT name FROM sqlite_master WHERE type = 'table'").all() as {
        name: string
      }[]
    ).map((row) => row.name)
    staged.close()
  ...
  it('replaces a staged import left over from an earlier attempt', () => {
    ...
    const staged = new Database(stagedPath)
    expect(staged.pragma('user_version', { simple: true })).toBe(MIGRATIONS.length)
    staged.close()
  })
})
...
describe('applyStagedImport', () => {
  ...
  it('backs up the current database, swaps in the import, and clears the sidecars', () => {
    seedDatabase(dbPath, (db) => {
      db.prepare('INSERT INTO channels (name, number) VALUES (?, ?)').run('Current', 7)
    })
    ...
    const source = join(dir, 'backup.db')
    seedDatabase(source, (db) => {
      db.prepare('INSERT INTO channels (name, number) VALUES (?, ?)').run('Imported', 42)
    })
    inspectAndStage(source, stagedPath, metaPath)
    ...
    const channel = live.prepare('SELECT name FROM channels').get() as { name: string }
    live.close()
    ...
    const savedChannel = saved.prepare('SELECT name FROM channels').get() as { name: string }
    saved.close()
...
describe('the boot sequence', () => {
  it('applies, opens, and records the receipt the way bootstrap() does', () => {
    ...
    const channel = db.prepare('SELECT name FROM channels').get() as { name: string }
    expect(channel.name).toBe('Imported')
